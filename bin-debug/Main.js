...
var Main = (function (_super) {
    ...
    p.notify = function (event) {
        this.observerList.forEach(function (observer) { observer.onChange(event); });
    };
    p.autoButton = function () {
    ...
    p.showHeroPanel = function () {
        var _this = this;
        var userPanel = new HeroPanel(this.player.curHero);
        this.addChild(userPanel);
        this.player.touchEnabled = false;
        userPanel.close.addEventListener(egret.TouchEvent.TOUCH_TAP, function () {
            _this.removeChild(userPanel);
...
egret.registerClass(Main,'Main',["EventEmitter"]);
